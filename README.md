...
For place & route, all ports should be aligned on the intersections [2] of a grid. To show the grid for high-density cells (`sky130_fd_sc_hd__...`) type `grid 0.46um 0.34um 0.23um 0.17um` into the magic TCL-console. 
...
Background: LEF/DEF Language Reference [6] `LEFclass CORE` specifies the cell as a standard cell used in the core-area. `LEFsource USER` means the cell is generated by the user for user-defined reasons. `LEFsite unithd` defines a placement grid for a family of macros, such as `CORE`. `unithd` refers to the sky130 high denity cell unit site, and it is defined in the technology LEF of the PDK. 
...
[6] LEF/DEF Definition: https://web.archive.org/web/20181031162337/http://www.ispd.cc/contests/18/lefdefref.pdf
