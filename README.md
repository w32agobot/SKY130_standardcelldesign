...
Background: LEF/DEF Language Reference [6] `LEFclass CORE` specifies the cell as a standard cell used in the core-area. `LEFsite unithd` defines a placement grid for a family of macros, such as `CORE`. `unithd` refers to the sky130 high denity cell unit site, and it is defined in the technology LEF of the PDK. 
